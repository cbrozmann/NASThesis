...
def get_valid_split(names, split_by='P', cross_subject=True):
    ...
    for i in range(len(names)):
        ...
        if split_by == 'S':
            ...
            # split by performer/subject (1-40)
            # available ids for the train set: [1, 2, 4, 5, 8, 9, 13, 14, 15, 16, 17, 18, 19, 25, 27, 28, 31, 34, 35, 38]
            if split_id % 3 == 0:
                split_names.append(name)
                split_ids.append(i)
    ...
    if split_by == 'S':
        # split by setup number (1-17)
        valid = [f'S{i:03d}' for i in range(3, 18, 3)]
        print(f'Split by "{split_by}" (setup number)')
        print(f'Get all images with {valid}')
    elif split_by == 'P':
        # split by performer/subject (1-40)
        if cross_subject:
            valid = [f'P{i:03d}' for i in [1, 2, 4, 5, 8, 9, 13, 14, 15, 16, 17, 18, 19, 25, 27, 28, 31, 34, 35, 38] if i % 3 == 2]
        else:
