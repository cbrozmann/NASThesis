...
#
# ./scripts/NATS-Bench/own_train-topology.sh 00000-15624 12 777
##############################################################################
echo script name: $0
echo $# arguments
if [ "$#" -ne 3 ] ;then
...
save_dir=./output/own/utd_mhad/train/NATS-Bench-topology/

OMP_NUM_THREADS=${cpus} python exps/NATS-Bench/own_main-tss_no_augmentation.py \
	--mode new --srange ${srange} --hyper ${opt} --save_dir ${save_dir} \
	--datasets utdmhad \
	--splits   1 \
	--xpaths $TORCH_HOME/UTD-MHAD \
	--workers ${cpus} \
	--seeds ${all_seeds} --config 1
/bin/bash