...
from xautodl.datasets import get_datasets, get_datasets_no_augmentation
...
def evaluate_all_datasets(
    ...
    for dataset, xpath, split in zip(datasets, xpaths, splits):
        # train valid data
        train_data, valid_data, xshape, class_num = get_datasets_no_augmentation(dataset, xpath, -1)
        # load the configuration
        if dataset == 'utdmhad':    # new start
            num_images = 431  # split the train_data
            # train_split = [x for x in range(num_images) if not x % 4 == 2]
            # valid_split = [x for x in range(num_images) if x % 4 == 2]
        ...
        elif dataset in ['ntu60_cross_subject', 'ntu60_cross_view']:  # new start
            if dataset == 'ntu60_cross_subject':
                num_images = 40091  # split the test_data
                dataset_path = 'configs/nas-benchmark/ntu_60_coss_subject-subject_split.json'
            else:
                num_images = 37646  # split the test_data
                dataset_path = 'configs/nas-benchmark/ntu_60_coss_view-subject_split.json'
            with open(dataset_path, 'r') as f:
                split_info = json.load(f)
        elif dataset == 'ntu120_cross_subject':
            num_images = 63026
            dataset_path = 'configs/nas-benchmark/ntu_120_coss_subject-subject_split.json'
            with open(dataset_path, 'r') as f:
                split_info = json.load(f)
        elif dataset == "cifar10" or dataset == "cifar100":  # if -> elif
        ...
        if bool(split):
            ...
            elif dataset == 'utdmhad' or dataset.startswith('ntu'):
                ValLoaders = {
                    "ori-test": torch.utils.data.DataLoader(
                        valid_data,
                        batch_size=config.batch_size,
            ...
            if dataset == 'utdmhad':  # new start
                num_images = 430  # split the test_data
                # subject id [2, 6] in valid split and [4,8] in test split
                valid_split = [x for x in range(0, 363) if x % 8 < 4] + [x for x in range(367, 430) if
                                                                         x % 8 < 3 or x % 8 == 7]
                test_split = [x for x in range(0, 430) if x not in valid_split]
                ValLoaders = {
...
if __name__ == "__main__":
    ...
    if args.mode == "meta":
        ...
        if args.config == 1:
            add_opt_path = '_own'
        opt_config = "./configs/nas-benchmark/hyper-opts/{:}E{:}.config".format(args.hyper, add_opt_path)
