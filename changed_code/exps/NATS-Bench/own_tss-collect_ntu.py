...
from nats_bench import pickle_save, pickle_load, ArchResults, ResultsCount, create
from nas_201_api import NASBench201API

# in default path
# api = NASBench201API(None)
api = create(None, 'tss', fast_mode=True)
NATS_TSS_BASE_NAME = "NATS-tss-v1_0"  # 2020.08.28


def update_eval_acc5(self, acc5s):
    """To update the evaluataion results."""
    data_names = set([x.split("@")[0] for x in acc5s.keys()])
    for data_name in data_names:
        ...
        for iepoch in range(self.epochs):
            ...
            self.eval_acc5es[xkey] = acc5s[xkey]
...
def create_result_count(
    ...
    if "train_times" in results:  # new version
        ...
        xresult.eval_acc5es = {}
        update_eval_acc5(xresult,results["valid_acc5es"])
    else:
        network = get_cell_based_tiny_net(net_config)
        network.load_state_dict(xresult.get_net_param())
        # start new
        if dataset == 'ntu60_cross_subject-valid':
            ...
            xresult.update_latency(latencies)  # end new
        else:
    ...
    for checkpoint_path in checkpoints:
        ...
        for dataset in datasets:
            ...
            arch_config = {
                ...
                "arch_str": arch_str,
                "class_num": results["opt_config"]["class_num"],
            }
...
def simplify(save_dir, save_name, nets, total, sup_config):
    ...
    datasets = ('ntu60_cross_subject-valid', 'utdmhad-valid')
